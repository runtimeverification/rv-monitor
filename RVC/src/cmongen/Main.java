/*
 * entry class for cmongen - based somewhat off the MOP logic repository code,
...
import cmongen.shells.*;
import cmongen.shells.fsm.*;
...
public class Main {
  ...
  static public void main(String[] args) {
    ...
    try{
      ...
      String mainClassPath = loader.getResource("cmongen/Main.class").toString();
      String cmgPath = null;
      if (mainClassPath.endsWith(".jar!/cmongen/Main.class") && mainClassPath.startsWith("jar:")) {
        cmgPath = mainClassPath.substring("jar:file:".length(), mainClassPath.length()
                   - "cmongen.jar!/cmongen/Main.class".length());
        cmgPath = polishPath(cmgPath);
        isJarFile = true;
        jarFilePath = mainClassPath.substring("jar:file:".length(), mainClassPath.length()
                       - "!/cmongen/Main.class".length());
