 ...
 *
 * As its name implies, an instance of this class represents an interface, not an
 * implementation (IndexingTreeImplementation). That is, two different instances of
 * this class can share the same implementation. By default, there is one-to-one
 * mapping between an instance of this class and an instance of IndexingTreeImplementation,
 * but this becomes no longer true when indexing trees are combined.
 * For example, suppose that an indexing tree for [Collection, Iterator] and another tree
 * for [Collection] are combined. Before combining them, there are two different interfaces
 * and they have their own implementations; i.e.,
 *   itf_[C,I] -> impl_[C,I]
 *   itf_[C]   -> impl_[C]
 *
 * The combining procedure will merge two implementations and modify the relation as follows:
 *   itf_[C,I] -> impl_[C,I]
 *   itf_[C]   /
 *
 * In the generated code, only one indexing tree will be instantiated because two interfaces
 * share the same implementation. However, two distinct caches will be generated because there
 * are two different ways to access the shared implementation; e.g., there are a cache for [C,I]
 * and another for [C].
 * 
 * Besides, the separation of interfaces and implementations makes the code generator simpler,
 * I think.
 * 
...
public class IndexingTreeInterface implements ICodeGenerator {
	 ...
	 * The condition is:
	 * 1. two indexing trees belong to the same specification because combining
	 * trees among different specifications has not been implemented and can be
	 * tricky.
	 * 2. two trees share the same prefixes; e.g., [C,I] and [C] can be merged,
	 * but [C,I] and [I] cannot be.
	 * @param that the other indexing tree that can be possibly part of this tree
	...
	public CodeStmtCollection generateFindCode(CodeExpr rootref, Access access, WeakReferenceVariables weakrefs, StmtCollectionInserter<CodeExpr> inserter) {
		return this.impl.generateFindCode(rootref, this.queryParams, access, this.specParams, weakrefs, inserter);
	}
