...
/**
 * This interface defines operations that any indexing tree visitor should implement,
 * such as what the visitor should do when it reaches the last entry and what it should
 * do before/after it reaches each entry.
 * A visitor, an implementation of this interface, is used to generate code.
 * 
...
public interface IIndexingTreeVisitor {
	 ...
	 * @param entry entry that is about to be visited
	 * @param parentref reference to the parent entry
	 * @param weakref reference to the weak reference that directs to this entry
	 * @param entryref reference to the entry
	 * @param nextparam next parameter in the indexing tree
	 ...
	 * @param entry entry that has been visited
	 * @param entryref reference to the entry
	 * @param nextparam next parameter in the indexing tree
	 * @param nested code generated while visiting the node
	 * @return generated code
	 */
	...
	/**
	 * Invoked when the second-last node is reached. This moment may be important to
	 * insert a new monitor into a set or map because the last entry would be the
	 * monitor itself.
	 * @param entry the second-last node
	 * @param entryref reference to the entry
	 * @return generated code
	 */
...
abstract class GenerativeIndexingTreeVisitor implements IIndexingTreeVisitor {
	...
	private CodeNewExpr createSet(Entry nextentry, RVMParameter curparam) {
		...
		if (settype instanceof CodeRVType.PartitionedMonitorSet) {
			...
			List<IndexingTreeInterface> trees = pset.getConstructorArguments(curparam);
