...
class SpecContext {
	...
	public SpecContext(RVMonitorSpecExt spec, RVMSpecFileExt currentFile, HashMap<String, RVMSpecFileExt> depFiles) {
		this.spec = spec;
...
public class RVMonitorExtender {
	...
	protected static RVMonitorSpec translateJavaMopSpec(RVMonitorSpecExt spec, RVMSpecFileExt currentFile, HashMap<String, RVMSpecFileExt> depFiles)
		...
		RVMonitorSpec ret;
		try {
			ret = new RVMonitorSpec(currentFile.getPakage(), spec.getBeginLine(), spec.getBeginColumn(), spec.getModifiers(), spec.getName(), spec.getParameters().toList(),
	...
	protected static EventDefinition translateEvent(EventDefinitionExt event, SpecContext context) throws RVMException {
		...
		try {
			ret = new EventDefinition(event.getBeginLine(), event.getBeginColumn(), event.getId(), event
					.getParameters().toList(), event.getBlock(), event.isStartEvent(), event.isBlockingEvent());
		} catch (Exception e) {
			throw new RVMException(e.getMessage());
		}
		return ret;
	...
	private static HashMap<String, RVMSpecFileExt> retrieveParentFiles(RVMonitorSpecExt spec, RVMSpecFileExt currentFile) throws RVMException {
		...
		for (ExtendedSpec extSpec : spec.getExtendedSpec()) {
			...
			for (RVMonitorSpecExt spec2 : parentSpecFile.getSpecs()) {
				HashMap<String, RVMSpecFileExt> temp = retrieveParentFiles(spec2, parentSpecFile);
				ret.putAll(temp);
			}
		}
	...
	private static RVMonitorSpecExt findRVMonitorSpec(String name, RVMSpecFileExt currentFile) {
		for (RVMonitorSpecExt spec : currentFile.getSpecs()) {
	...
	private static Pair<RVMonitorSpecExt, RVMSpecFileExt> findRVMonitorSpec(String name, RVMSpecFileExt currentFile,
																			HashMap<String, RVMSpecFileExt> depFiles) throws RVMException {
		RVMonitorSpecExt parentSpec = findRVMonitorSpec(name, currentFile);

		if (parentSpec != null)
			return new Pair<RVMonitorSpecExt, RVMSpecFileExt>(parentSpec, currentFile);
		...
		if (specFile != null) {
			...
			if (parentSpec != null)
				return new Pair<RVMonitorSpecExt, RVMSpecFileExt>(parentSpec, specFile);
		}
	...
	private static List<EventDefinitionExt> collectNonImpAbstractEvents(RVMonitorSpecExt spec, RVMSpecFileExt currentFile,
			HashMap<String, RVMSpecFileExt> depFiles) throws RVMException {
