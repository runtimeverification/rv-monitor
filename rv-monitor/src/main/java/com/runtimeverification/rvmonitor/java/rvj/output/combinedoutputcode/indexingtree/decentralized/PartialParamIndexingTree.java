...
public class PartialParamIndexingTree extends IndexingTree {
	...
	public PartialParamIndexingTree(String outputName, RVMParameters queryParam, RVMParameters contentParam, RVMParameters fullParam, MonitorSet monitorSet, SuffixMonitor monitor,
	...
	protected String lookupIntermediateCreative(LocalVariables localVars, RVMVariable monitor, RVMVariable lastMap, RVMVariable lastSet, int i, int target) throws RVMException {
		String ret = "";
	...
	protected String lookupIntermediateNonCreative(LocalVariables localVars, RVMVariable monitor, RVMVariable lastMap, RVMVariable lastSet, int i, int target) throws RVMException {
		String ret = "";
	...
	protected String lookupNodeAndSetLast(LocalVariables localVars, RVMVariable monitor, RVMVariable lastMap, RVMVariable lastSet, int i, boolean creative)  throws RVMException {
		String ret = "";
	...
	protected String createNewMap(int paramIndex) throws RVMException {
		...
		if(isGeneral){
			if (paramIndex == queryParam.size() - 1) {
				ret += "new com.runtimeverification.rvmonitor.java.rt.map.RVMMapOfSetMon(" + fullParam.getIdnum(queryParam.get(paramIndex)) + ")";
			} else {
				ret += "new com.runtimeverification.rvmonitor.java.rt.map.RVMMapOfAll(" + fullParam.getIdnum(queryParam.get(paramIndex)) + ")";
			}
		} else {
			if (paramIndex == queryParam.size() - 1) {
				ret += "new com.runtimeverification.rvmonitor.java.rt.map.RVMMapOfSet(" + fullParam.getIdnum(queryParam.get(paramIndex)) + ")";
			} else {
	...
	public String toString() {
		...
		if(isGeneral){
			if (queryParam.size() == 2) {
				ret += "com.runtimeverification.rvmonitor.java.rt.map.RVMAbstractMap " + firstKey.getType() + "." + name + " = null;\n";
			} else {
				ret += "com.runtimeverification.rvmonitor.java.rt.map.RVMAbstractMap " + firstKey.getType() + "." + name + " = null;\n";
			}
		} else {
