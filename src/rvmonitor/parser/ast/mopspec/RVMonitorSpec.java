...
import rvmonitor.parser.ast.visitor.CheckThisJoinPointVisitor;
...
public class RVMonitorSpec extends Node implements Comparable<RVMonitorSpec>{
	...
	public RVMonitorSpec(PackageDeclaration packageDeclaration, int line, int column, int modifiers, String name, List<RVMParameter> parameters, String inMethod, List<BodyDeclaration> declarations,
						 List<EventDefinition> events, List<PropertyAndHandlers> properties) throws rvmonitor.parser.main_parser.ParseException {
		super(line, column);
		...
		for (PropertyAndHandlers prop : properties){
			for(String category: prop.getHandlers().keySet()){
				RVMParameters param = prop.getUsedParametersIn(category, this.parameters);
		...
		for (String eventName : duplicatedEventNames) {
			...
			for (EventDefinition event : this.events) {
				if (event.getId().equals(eventName)) {
					while (RVMNameSpace.checkUserVariable(event.getId() + "_" + idnum))
						idnum++;
