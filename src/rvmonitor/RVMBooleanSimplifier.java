...
class RVMTruthTable {
	...
	RVMTruthTable trueTable;
	RVMTruthTable falseTable;

	// 0 = false, 1 = true, 2 = true and covered
	int trueEntry = 0;
	int falseEntry = 0;

	public RVMTruthTable(int level) {
		this.level = level;

		if (level > 1) {
			trueTable = new RVMTruthTable(level - 1);
			falseTable = new RVMTruthTable(level - 1);
		}
	}
...
public class RVMBooleanSimplifier {
	...
	public static RVMParameterSet simplify(RVMParameterSet paramSet, RVMParameters fullParam) {
		...
		RVMTruthTable truthTable = new RVMTruthTable(numParam);
