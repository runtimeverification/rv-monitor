...
public class Main {
	...
	static String rvmonitorDir = null;
	...
	public static void print_help() {
		System.out.println("Usage: java [-cp javmaop_classpath] rvmonitor.TestSuite [-options] <test suite dir>");
		System.out.println("");
		...
		System.out.println("    -m <rvmonitor dir path>\t  select the rvmonitor directory");
		System.out.println();
	}
...
//			System.out.println();
	...
	public static void main(String[] args) {
		...
		String mainClassPath = loader.getResource("rvmonitor/Main.class").toString();
		if (mainClassPath.endsWith(".jar!/rvmonitor/Main.class") && mainClassPath.startsWith("jar:")) {
			isJarFile = true;

			jarFilePath = mainClassPath.substring("jar:file:".length(), mainClassPath.length() - "!/rvmonitor/Main.class".length());
		...
		if (Main.rvmonitorDir == null || Main.rvmonitorDir.length() == 0) {
			if (Main.isJarFile)
				Main.rvmonitorDir = "";
			else
				Main.rvmonitorDir = base_path + File.separator;
		}
		...
		try {
			...
			if (!Main.isJarFile)
				Main.rvmonitorDir = (new File(Main.rvmonitorDir).getCanonicalPath());
		} catch (Exception e) {
		}
		...
		if (!Main.isJarFile && !new File(Main.rvmonitorDir).exists()) {
			System.err.println("[ERROR] the given rvmonitor directory does not exist.");
			return;
		}
		try {
