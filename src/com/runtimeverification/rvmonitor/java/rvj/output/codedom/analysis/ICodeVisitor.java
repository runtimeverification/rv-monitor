...
/**
 * This class implements a visitor that eliminates the given junks
 * from all the visited statements.
 *
 * @author Choonghwan Lee <clee83@illinois.edu>
 */
class StmtEliminationVisitor implements ICodeVisitor {
	/**
	 * Keeps all the remaining junk statements. Initially,
	 * junk statements are given by the caller.
	 */
	...
	static class Block {
		...
		public boolean assign(CodeAssignStmt assign) {
			...
			if (lhs instanceof CodeVarRefExpr) {
				...
				CodeVarDeclStmt decl = declaring.declared.get(assigned.getName());
		...
		private Block searchRecursively(CodeVarDeclStmt decl) {
			for (Map.Entry<String, CodeVarDeclStmt> entry : this.declared.entrySet()) {
		...
		public void collectUnusedDeclarationsAndAssignmentsRecursively(Set<CodeStmt> junks) {
			for (Map.Entry<String, CodeVarDeclStmt> entry : this.declared.entrySet()) {
				if (!this.referred.contains(entry.getValue())) {
					junks.add(entry.getValue());
					List<CodeAssignStmt> assigns = this.assigned.get(entry.getValue());
					if (assigns != null)
