...
/**
 * This class takes statements (an instance of CodeStmtCollection) and simplifies
 * them. For example, while generating code at the first stage, it is not trivial
 * to see whether or not a variable will be actually referred. Instead of creating
 * only necessary variables, RV-Monitor first creates all possibly used variables
 * and then uses this class to eliminate unused variables.
 * 
 * Currently, this class does the following simplification:
 * 1. Unused variable elimination: if a variable is defined but never used, all
 * the relevant code is eliminated.
 * 
 * The followings should be done as well:
 * 1. Empty body elimination: if the body of a branch or loop is empty, that block
 * is eliminated.
 * 2. Redundant check elimination: if the same check is performed without any
 * potential update, the redundant check is eliminated.
 * 
...
public class CodeSimplifier {
	...
	class UnusedVariableElimination {
		...
		private Set<CodeStmt> collect() {
			Set<CodeStmt> junks = CodeSimplifier.this.current.visitor.collectUnusedDeclarationsAndAssignments();
			return junks;
		}
		...
		private void eliminate(Set<CodeStmt> junks) {
			...
			if (remaining.size() > 0) {
				// This probably indicates that some code was unreachable due to some missing
