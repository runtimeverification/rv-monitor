...
public class MonitorSet {
	...
	/**
	 * Initializes the 'treemap' field based on the provided argument.
	 * This method does not assume that each indexing tree has its own
	...
	public void feedIndexingTreeInterface(IndexingDeclNew indexingTreeDecl) {
		if (this.usePartitionedSet) {
			...
			TreeMap<RVMParameters, IndexingTreeInterface> trees = indexingTreeDecl.getIndexingTrees();
			for (Map.Entry<RVMParameters, IndexingTreeInterface> entry : trees.entrySet()) {
	...
	/**
	 * Initializes the 'treefields' field based on the provided argument.
	 * This method assumes that implementations of indexing trees are initialized.
	 * @param indexingTreeDecl
	 */
	public void feedIndexingTreeImplementation(IndexingDeclNew indexingTreeDecl) {
		if (this.usePartitionedSet) {
			...
			for (Map.Entry<RVMParameters, IndexingTreeInterface> entry : trees.entrySet()) {
				...
				IndexingTreeInterface treeitf = entry.getValue();
				CodeMemberField treefield = treeitf.getImplementation().getField();
				CodeMemberField setfield = new CodeMemberField(treefield.getName(), false, false, true, treefield.getType());
	...
	/**
	 * Initializes the 'handlerfields' field. This method assumes that the 'treefields' field
	 * is initialized.
	 */
	private void initializeHandlerFields() {
		if (this.usePartitionedSet) {
			...
			for (EventDefinition evt : this.events) {
				...
				CodeType type = CodeHelper.RuntimeType.getSetEventDelegator(monitortype, pairmaptype);
				String name = evt.getId() + "Handler";
				CodeMemberField field = new CodeMemberField(name, false, true, true, type);
	...
	private boolean determineIfPartitionedSetApplicable() {
		...
		// This is necessary, in the current assumption, because the set corresponding
		// to the other parameter should be invalidated. I believe this restriction
		// can be relaxed.
		for (EventDefinition evt : this.events) {
			if (!evt.isStartEvent()) {
		...
		// Also, I assume that there is at least one event that carries each parameter only.
		{
				...
				if (!evt.isStartEvent()) {
					...
					if (evtprms.size() == 1) {
						if (accumulated == null)
							...
							accumulated = RVMParameters.unionSet(accumulated, evtprms);
					}
				}
			}
	...
	public CodeStmtCollection generateMonitoringCode(CodeVarRefExpr setref, EventDefinition event, GlobalLock enforcelock) {
		 ...
		 * code, which might do something important, let's use the legacy code.
		String methodname = "event_" + event.getId();
		...
		for (RVMParameter param : passing)
			args.add(CodeExpr.fromLegacy(CodeType.object(), param.getName()));
		CodeExpr invoke = new CodeMethodInvokeExpr(CodeType.foid(), setref, methodname, args);
...
//		}
	...
	public CodeRVType.MonitorSet getCodeType() {
		...
		if (this.usePartitionedSet) {
			...
			for (Map.Entry<RVMParameter, IndexingTreeInterface> entry : this.treemap.entrySet()) {
	...
	private void generatePartitionedSetConstructorBody(ICodeFormatter fmt) {
		...
		CodeExpr invokesuper = new CodeMethodInvokeExpr(CodeType.foid(), null, "super",
			new CodeMethodInvokeExpr(CodeType.integer(), monitortype, null, "getNumberOfStates"),
			new CodeMethodInvokeExpr(CodeType.integer(), monitortype, null, "getNumberOfEvents"));
		...
		for (CodeMemberField field : this.treefields.values()) {
			CodeStmt assign = new CodeAssignStmt(
				new CodeFieldRefExpr(new CodeThisRefExpr(settype), field),
				new CodeVarRefExpr(new CodeVariable(field.getType(), field.getName())));
			stmts.add(assign);
		}
		
		// Register this set to the TerminatedMonitorCleaner, so that unnecessary monitors in the set
		// can be reclaimed.
		{
			CodeType cleanertype = CodeHelper.RuntimeType.getTerminatedMonitorCleaner();
			CodeExpr lockref = new CodeFieldRefExpr(this.typewiseLock);
			CodeExpr addset = new CodeMethodInvokeExpr(CodeType.foid(), cleanertype, null, "addSet", new CodeThisRefExpr(this.getCodeType()), lockref);
	...
	private CodeMemberStaticInitializer generatePartitionedSetStaticInitializer() {
		...
		// Initialize handlers.
		for (Map.Entry<EventDefinition, CodeMemberField> entry : this.handlerfields.entrySet()) {
			EventDefinition evt = entry.getKey();
			CodeMemberField field = entry.getValue();
			CodeClassDef klass = this.generateHandlerClass(evt, field.getType());
			CodeExpr create = new CodeNewExpr(field.getType(), klass);
			CodeStmt assign = new CodeAssignStmt(new CodeFieldRefExpr(field), create);
			stmts.add(assign);
		}
	...
	private CodeClassDef generateHandlerClass(EventDefinition evt, CodeType klasstype) {
		...
		// Defines commit().
		{
			...
			if (!evt.isStartEvent()) {
				...
				WeakReferenceVariables weakrefs = new WeakReferenceVariables(this.indexingTreeManager, pairparam);

				CodeMemberField weakreffield = CodeHelper.VariableName.getWeakRefInMonitor(pairparam, CodeHelper.RuntimeType.getWeakReference());
				CodeVariable weakrefvar = weakrefs.getWeakRef(pairparam);
				body.add(new CodeVarDeclStmt(weakrefvar, new CodeFieldRefExpr(monitorref, weakreffield)));

				IndexingTreeInterface pairtree = this.treemap.get(pairparam);
//				CodeFieldRefExpr pairmapref = new CodeFieldRefExpr(new CodeThisRefExpr(klasstype), pairmapfield);
				StmtCollectionInserter<CodeExpr> inserter = new StmtCollectionInserter<CodeExpr>() {
					@Override
					public CodeStmtCollection insertLastField(Entry entry, CodeExpr setref) {
						CodeExpr invalidate = new CodeMethodInvokeExpr(CodeType.foid(), setref, "invalidate", monitorref);
						return new CodeStmtCollection(new CodeExprStmt(invalidate));
					}
				};
				body.add(pairtree.generateFindCode(pairmapref, Access.Set, weakrefs, inserter));
			}

			CodeMemberMethod method = new CodeMemberMethod("commit", true, false, true, CodeType.foid(), true, body,
	...
	private CodeStmtCollection generateSynchronizedBlock(CodeStmtCollection body) {
		// To use more professional lock operations, such as tryLock(), synchronized
		// blocks do not seem proper.
		boolean useclasslock = false;

		if (useclasslock) {
			// synchronized (type.class) { ... }
			CodeExpr lockobj = new CodeFieldRefExpr(this.getCodeType(), new CodeMemberField("class", true, false, true, CodeType.klass()));
			...
			{
				CodeExpr lock = new CodeMethodInvokeExpr(CodeType.foid(), new CodeFieldRefExpr(this.typewiseLock), "lock");
			...
			{
				CodeExpr unlock = new CodeMethodInvokeExpr(CodeType.foid(), new CodeFieldRefExpr(this.typewiseLock), "unlock");
				finallyblock.add(new CodeExprStmt(unlock));
			}

			CodeStmt tryfinallyblock = new CodeTryCatchFinallyStmt(tryblock, finallyblock);
	...
	private void generateTerminateMethod(ICodeFormatter fmt) {
		...
		{
			CodeExpr invokesuper = new CodeMethodInvokeExpr(CodeType.foid(), new CodeThisRefExpr(this.getCodeType()), "terminateInternal", new CodeVarRefExpr(param1));
			body.add(new CodeExprStmt(invokesuper));

			CodeType cleanertype = CodeHelper.RuntimeType.getTerminatedMonitorCleaner();
			CodeExpr addset = new CodeMethodInvokeExpr(CodeType.foid(), cleanertype, null, "removeSet", new CodeThisRefExpr(this.getCodeType()));
			body.add(new CodeExprStmt(addset));
		}

		CodeMemberMethod method = new CodeMemberMethod("terminate", true, false, true, CodeType.foid(), true, body, param1);
		method.getCode(fmt);
	}

	private void generateAddMethod(ICodeFormatter fmt) {
		CodeVariable param1 = new CodeVariable(this.getMonitorCodeType(), "monitor");
		
		CodeExpr invokeadd = new CodeMethodInvokeExpr(CodeType.foid(), new CodeThisRefExpr(this.getMonitorCodeType()), "addUnprotected", new CodeVarRefExpr(param1));
		CodeStmtCollection body = this.generateSynchronizedBlock(new CodeExprStmt(invokeadd));
		
		CodeMemberMethod method = new CodeMemberMethod("add", true, false, true, CodeType.foid(), false, body, param1);
		method.getCode(fmt);
	}
	...
	private CodeStmtCollection generateEventHandler(EventDefinition evt) {
		...
		if (evt.isStartEvent())
			...
			arg2 = new CodeFieldRefExpr(new CodeThisRefExpr(this.getCodeType()), pairmapfield);
		}
		CodeExpr invoke = new CodeMethodInvokeExpr(CodeType.foid(), target, "fireEvent", arg1, arg2);
		CodeStmtCollection body = new CodeStmtCollection(new CodeExprStmt(invoke));
	...
	private Map.Entry<RVMParameter, CodeMemberField> getPair(EventDefinition evt) {
		// Since we apply the partition-set optimization only when there are two parameters,
		// finding the pair set is straightforward.
		for (Map.Entry<RVMParameter, CodeMemberField> entry : this.treefields.entrySet()) {
	...
	private List<RVMParameter> getSortedParameterList() {
		ArrayList<RVMParameter> prmlist = new ArrayList<RVMParameter>(this.treemap.keySet());
...
//		RVMVariable num_terminated_monitors = new RVMVariable("num_terminated_monitors");
		...
		if (this.usePartitionedSet)
			ret += "com.runtimeverification.rvmonitor.java.rt.tablebase.AbstractPartitionedMonitorSet<" + monitorName + ">";
		else
			ret += "com.runtimeverification.rvmonitor.java.rt.tablebase.AbstractMonitorSet<" + monitorName + ">";
		ret += " {\n";
...
//			ret += "String " + loc + " = null;\n";
		...
		if (this.usePartitionedSet) {
			...
			CodeMemberMethod property = new CodeMemberMethod("getLock", true, true, false, field.getType(), false, get);
		...
		if (this.usePartitionedSet) {
			ICodeFormatter fmt = CodeFormatters.getDefault();
			CodeMemberStaticInitializer init = this.generatePartitionedSetStaticInitializer();
			init.getCode(fmt);
			ret += fmt.getCode();
		}
		...
		for (EventDefinition event : this.events) {
			...
			if (Main.stripUnusedParameterInMonitor)
				parameters = event.getReferredParameters(event.getRVMParameters());
			else
				parameters = event.getRVMParameters();

			ret += "final" + (synch ? " synchronized " : " ") + "void event_" + eventName + "(";
			...
			if (this.usePartitionedSet) {
				...
				ret += "int " + numAlive + " = 0 ;\n";
				ret += "for(int " + loopindex + " = 0; " + loopindex + " < this.size; " + loopindex + "++){\n";
				ret += monitorName + " " + monitor + " = this.elements[" + loopindex + "];\n";
				...
				ret += "for(int " + loopindex + " = " + numAlive + "; " + loopindex + " < this.size; " + loopindex + "++){\n";
