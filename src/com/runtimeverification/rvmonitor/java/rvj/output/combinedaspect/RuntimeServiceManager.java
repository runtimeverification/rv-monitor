...
public class RuntimeServiceManager implements ICodeGenerator {
	...
	private ServiceDefinition addCleanerService() {
		...
		{
			CodeType type = CodeHelper.RuntimeType.getTerminatedMonitorCleaner();
			CodeExpr start = new CodeMethodInvokeExpr(CodeType.foid(), type, null, "start");
	...
	private ServiceDefinition addObserverService() {
		...
		{
			CodeFieldRefExpr fieldref = new CodeFieldRefExpr(this.observer.getField());
			// getObservable()
			{
				CodeType rettype = CodeHelper.RuntimeType.getObserverable(CodeHelper.RuntimeType.getInternalBehaviorObserver());
				CodeStmt body = new CodeReturnStmt(fieldref);
				CodeMemberMethod method = new CodeMemberMethod("getObservable", true, true, true, rettype, false, body);
				methods.add(method);
			}
			// subscribe(o)
			{
				CodeVariable param1 = new CodeVariable(CodeHelper.RuntimeType.getInternalBehaviorObserver(), "o");
				CodeExpr invoke = new CodeMethodInvokeExpr(CodeType.foid(), fieldref, "subscribe", new CodeVarRefExpr(param1));
				CodeStmt body = new CodeExprStmt(invoke);
				CodeMemberMethod method = new CodeMemberMethod("subscribe", true, true, true, CodeType.foid(), false, body, param1);
				methods.add(method);
			}
			// unsubscribe(o)
			{
				CodeVariable param1 = new CodeVariable(CodeHelper.RuntimeType.getInternalBehaviorObserver(), "o");
				CodeExpr invoke = new CodeMethodInvokeExpr(CodeType.foid(), fieldref, "unsubscribe", new CodeVarRefExpr(param1));
				CodeStmt body = new CodeExprStmt(invoke);
				CodeMemberMethod method = new CodeMemberMethod("unsubscribe", true, true, true, CodeType.foid(), false, body, param1);
				methods.add(method);
			}
		}
	...
	private ServiceDefinition addRuntimeBehaviorOption() {
		...
		CodeStmtCollection init = new CodeStmtCollection();
		{
			...
			CodeExpr invoke = new CodeMethodInvokeExpr(CodeType.foid(), type, null, "enableFineGrainedLock", enabled);
	...
	static class ServiceDefinition {
		...
		protected ServiceDefinition(String desc, List<CodeMemberField> fields, List<CodeMemberMethod> methods, CodeStmtCollection init) {
