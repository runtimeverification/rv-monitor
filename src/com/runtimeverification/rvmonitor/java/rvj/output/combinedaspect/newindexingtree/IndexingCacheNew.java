...
/**
 * This class represents a single-entry indexing tree cache.
 * As its name implies, the cache allows the caller to avoid
 * accessing the indexing tree, when this cache hits.
 * Since an indexing tree has multiple levels, the key of this cache
 * may consist of multiple objects, and each is a parameter carried
 * by the event. The value that is associated with the key is the node,
 * which can be a monitor, set, or tuple.
 * 
 * The current implements employs a single entry cache; i.e., only the
 * most recently accessed key-value pair is hold. A preliminary experience
 * showed that having multiple entries do not increase hit ratio significantly.
 *
...
public class IndexingCacheNew implements ICodeGenerator {
	...
	private IndexingCacheNew(TreeMap<RVMParameter, CodeCacheField> keys, Entry valueEntry, CodeCacheField valueField) {
		this.keys = keys;
	...
	public static IndexingCacheNew fromTree(String treename, IndexingTreeInterface itf) {
		...
		CodeCacheField field = CodeCacheField.create(fieldname, lastentry.getCodeType());
		
		return new IndexingCacheNew(keys, lastentry, field);
	}
	
	/**
	 * Generates code for comparing the key of this cache with the parameters carried
	 * by the event. The generated code can be used to check whether the cache hits.
	 * @return generated code
	 */
	public CodeExpr getKeyCompareCode() {
		CodeExpr prev = null;
		for (Map.Entry<RVMParameter, CodeCacheField> entry : this.keys.entrySet()) {
	 ...
	 * The generated code should guarantee that the stored value is always non-null.
	 * At the time of writing this code, non-null is guaranteed by the caller.
	 * @param valueref reference to the value to be stored
	 * @return generated code
	 */
	public CodeStmtCollection getCacheUpdateCode(CodeVarRefExpr valueref) {
		...
		for (Map.Entry<RVMParameter, CodeCacheField> entry : this.keys.entrySet()) {
			...
			RVMParameter param = entry.getKey();
			CodeVariable paramvar = new CodeVariable(field.getField().getType(), param.getName());
	...
	/**
	 * This class represents a field for holding one sub-key or associated value.
	 * For example, if the indexing tree is two-level, then the key would consists
	 * of two sub-keys, one for each parameter. One instance of this class holds one sub-key.
	 * Two different implementations are used: CodeOrdinaryCacheField and CodeTLSCacheField.
	 * The former stores the sub-key or value at a typical field, whereas the latter stores 
	 * it at a thread-local storage (TLS).
	 *
	 * @author Choonghwan Lee <clee83@illinois.edu>
	 */
	 ...
	 * is shared among all the threads. Any code that reads or updates this field
	 * should be careful about data race.
	 * @author Choonghwan Lee <clee83@illinois.edu>
	 */
	static class CodeOrdinaryCacheField extends CodeCacheField {
		CodeOrdinaryCacheField(String fieldname, CodeType type) {
			this.field = new CodeMemberField(fieldname, false, true, false, type);
		}
	...
	static class CodeTLSCacheField extends CodeCacheField {
		CodeTLSCacheField(String fieldname, CodeType type) {
			...
			this.field = new CodeMemberField(fieldname, false, true, false, tlstype, init);
		}

		@Override
		public CodeExpr generateGetCode() {
			CodeFieldRefExpr fieldref = new CodeFieldRefExpr(this.field);
			CodeExpr invoke = new CodeMethodInvokeExpr(CodeType.object(), fieldref, "get");
			return invoke;
		}
		...
		public CodeStmt generateSetCode(CodeVarRefExpr rhs) {
			CodeFieldRefExpr fieldref = new CodeFieldRefExpr(this.field);
			CodeExpr invoke = new CodeMethodInvokeExpr(CodeType.foid(), fieldref, "set", rhs);
