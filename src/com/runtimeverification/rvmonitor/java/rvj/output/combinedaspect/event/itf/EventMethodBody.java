	...
	private CodeStmtCollection generateMonitorTransition(IndexingTreeQueryResult matched) {
		...
		if (single) {
...

			{
				...
				String mntcode = this.monitorClass.Monitoring(monitorvar, this.event, null, null, this.enforceLock, this.getAspectName(), false);
				guarded.add(CodeStmtCollection.fromLegacy(mntcode));
			}
		}
		else {
			RVMVariable setvar = affectedref.getVariable().toLegacy();
			String mntcode = this.monitorSet.Monitoring(setvar, this.event, null, null, this.enforceLock);
			guarded.add(CodeStmtCollection.fromLegacy(mntcode));
		}
