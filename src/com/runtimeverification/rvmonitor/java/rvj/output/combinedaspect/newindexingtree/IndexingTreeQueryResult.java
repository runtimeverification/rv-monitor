...
public class IndexingTreeQueryResult {
	...
	public IndexingTreeQueryResult(IndexingTreeInterface tree, WeakReferenceVariables weakrefs, RVMParameters params, Access access, String prefix) {
		...
		if (params.size() > 0) {
			...
			CodeVariable var = new CodeVariable(secondlastmap.getCodeType(), varname);
	...
	
		{
			...
			CodeVariable var = new CodeVariable(this.matchedEntry.getCodeType(), varname);
			this.matchedEntryRef = new CodeVarRefExpr(var);
		}
		
		if (this.matchedEntry.getSet() != null) {
			String varname = prefix + "Set";
			CodeVariable var = new CodeVariable(this.matchedEntry.getSet(), varname);
		...
		if (this.matchedEntry.getLeaf() != null) {
			...
			CodeVariable var = new CodeVariable(this.matchedEntry.getLeaf(), varname);
	...
	public CodeStmtCollection generateDeclarationCode() {
		...
		if (this.secondLastMapRef != null)
			stmts.add(new CodeVarDeclStmt(this.secondLastMapRef.getVariable(), CodeLiteralExpr.nul()));

		stmts.add(new CodeVarDeclStmt(this.matchedEntryRef.getVariable(), CodeLiteralExpr.nul()));
		
		if (this.access == Access.Set && this.matchedSetRef != null)
			stmts.add(new CodeVarDeclStmt(this.matchedSetRef.getVariable(), CodeLiteralExpr.nul()));

		if (this.access == Access.Leaf && this.matchedLeafRef != null)
			stmts.add(new CodeVarDeclStmt(this.matchedLeafRef.getVariable(), CodeLiteralExpr.nul()));

		return stmts;
	}

	public CodePair<CodeVarRefExpr> generateFieldGetCode(Access access, String varnameprefix) {
		return this.matchedEntry.generateFieldGetCode(this.matchedEntryRef, access, varnameprefix);
	}

	public CodeStmtCollection generateFieldSetCode(Access access, CodeExpr value) {
		return this.matchedEntry.generateFieldSetCode(this.matchedEntryRef, access, value);
	}

	public CodeStmtCollection generateLeafUpdateCode(CodeVarRefExpr valueref, boolean forceleafupdate) {
		CodeStmtCollection stmts = new CodeStmtCollection();
