...
public class InternalBehaviorObservableCodeGenerator {
	...
	private CodeStmtCollection generateCommon(String methodname, CodeExpr ... args) {
		return this.generateCommon(methodname, Arrays.asList(args));
	}
	
	private CodeStmtCollection generateCommon(String methodname, List<CodeExpr> args) {
		if (!this.enabled) return CodeStmtCollection.empty();

		CodeExpr invoke = new CodeMethodInvokeExpr(CodeType.foid(), this.observerref, methodname, args);
	...
	public CodeStmtCollection generateEventMethodEnterCode(EventDefinition event) {
		...
		for (RVMParameter param : event.getRVMParametersOnSpec()) {
			CodeVariable var = new CodeVariable(CodeType.object(), param.getName());
	...
	public CodeStmtCollection generateIndexingTreeCacheHitCode(IndexingCacheNew cache, CodeVarRefExpr valueref) {
		CodeExpr cacheref = CodeLiteralExpr.string(cache.getValueField().getName());
		return this.generateCommon("onIndexingTreeCacheHit", cacheref, valueref);
	}

	public CodeStmtCollection generateIndexingTreeCacheMissedCode(IndexingCacheNew cache) {
		CodeExpr cacheref = CodeLiteralExpr.string(cache.getValueField().getName());
	...
	public CodeStmtCollection generateNewMonitorCreatedCode(CodeVarRefExpr monitorref) {
		return this.generateCommon("onNewMonitorCreated", monitorref);
	}

	public CodeStmtCollection generateMonitorClonedCode(CodeVarRefExpr sourcemonref, CodeVarRefExpr monitorref) {
		return this.generateCommon("onMonitorCloned", sourcemonref, monitorref);
	}

	public CodeStmtCollection generateDisableFieldUpdatedCode(CodeVarRefExpr holderref) {
		return this.generateCommon("onDisableFieldUpdated", holderref);
	}

	public CodeStmtCollection generateMonitorTransitionedCode(CodeVarRefExpr affectedref) {
