...
/**
 * This class specifies what features a monitor class should implement.
 * In general, a monitor instance should remember 'tau', which was a
 * misunderstanding of 't' in the paper, and weak references that this
 * instance is about. Also, it needs to keep track of 'disable'.
 * For some cases, however, these are unnecessary and, for the performance
 * benefit, it would be ideal to get rid of them.
 * 
 ...
 * enables the 'tau' feature, then the monitor generating class adds 'tau'
 * field to the generated code accordingly.
 * 
...
public class MonitorFeatures {
	...
	/**
	 * This field tells whether the generated monitor class should keep
	 * weak references for all parameters and define them as non-final.
	 * If this field is false, the following optimizations are applied:
	 * 1. Weak references for only unnecessary parameters are stored in
	 * a monitor instance, which saves memory consumption; and
	 * 2. The fields for holding weak references are defined as 'final',
	 * which may cause the JIT compiler to generate better code.
	 * If this field is true, the 'rememberedParameters' field should not
	 * be used.
	 * It should be noted that this field gets a stable value only after
	 * every EventMethodBody's generateCode() and this class'
	 * onCodeGenerationPass1Completed() have been invoked. Therefore, one
	 * needs to consider lazy code generation.
	 */
	 ...
	 * are never borrowed to clone monitor instances.
	 * This field should not be used if the 'needsNonFinalWeakRefsInMonitor' field
	 * is true; if that's the case, the generated monitor should consider all the
	 * parameters.
	 */
	...
	/**
	 * This field tells whether or not a place holder class for 'tau' and 'disable'
	 * is needed. When it is enabled, an interface, which is to be implemented by both
	 * the place holder and the generated monitor class, is to be created as well.
	 * For example, consider the example in the ASE'09 paper. If [e1<p1>, e3<p1,q1>] is observed,
	 * a monitor corresponding to <p1,q1> should be created. However, if [e1<p2>, e2<q2>, e3<p2,q2>]
	 * is observed, a monitor corresponding to <p2,q2> should not be created, due to the disable feature.
	 * In such case, a disable holder should be created instead. That is, the type of the leaf of the
	 * indexing tree for <P,Q> should be able to hold both a monitor and a disable holder, and this
	 * is where the interface is needed.
	 */
	...
	/**
	 * This method is to be invoked after every EventMethodBody instance's generateCode()
	 * has been invoked.
	 */
